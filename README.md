
# Toters

app of food delivery
search on youtube (how I can download project on GitHub)\
after you download the project you see the main folder in lib
https://www.youtube.com/watch?v=GHUKKFEIQX4

API\
costom\
data\
secren

API evre thing  of conect with API\
custom any widget I create it without using packages\
data som data I use it in files\
screen all app screen

## Badges

Add badges from somewhere like: [shields.io](https://img.shields.io/badge/dart-2.17.6-blue)


[![MIT License](https://img.shields.io/badge/dart-2.17.6-blue)](https://choosealicense.com/licenses/mit/)

[![GPLv3 License](https://img.shields.io/badge/flutter-3.0.5-blue)](https://opensource.org/licenses/)

[![GPLv3 License](https://img.shields.io/badge/node.js-16.17.0-green)](https://opensource.org/licenses/)

[![GPLv3 License](https://img.shields.io/badge/app__complet-70%25-success)](https://opensource.org/licenses/)



## License

[dart](https://dart.dev/)

[flutter](https://flutter.dev/)

[node.js](https://nodejs.org/en/)

[totersapp](https://www.totersapp.com/)

![Logo](data:image/png;base64,)






## pub.dev packages

#### Get all item

```http
http: ^0.13.5
```

| packages name| Description                |
| :--------  |:------------------------- |
| `http: ^0.13.5`| to get respons and send  requist to api |

#### Get item

```http
cupertino_icons: ^1.0.2
```

| packages name |  Description                       |
| :-------- | :-------------------------------- |
| `cupertino_icons`   |  to get item |

Takes two numbers and returns the sum.



## Screenshots


![App Screenshot](https://g.top4top.io/p_2453qzizu3.png)

![App Screenshot](https://a.top4top.io/p_2453ckwen8.png)

![App Screenshot](https://e.top4top.io/p_2453ana8e2.png)

![App Screenshot](  https://h.top4top.io/p_245305eqp4.png  )

![App Screenshot](    https://l.top4top.io/p_2453iw0le7.png)

![App Screenshot](   https://k.top4top.io/p_2453scmp86.png )

![App Screenshot](   https://j.top4top.io/p_24538jrt05.png )




